...
A scanned/keyed reference is treated as a **pickup** (issuing a `PICKUP` ticket on the regular 6000 series) when one of the following is true. Resolution lives in `src/services/pickup.service.ts`. No appointment_date gate applies to pickups, and cross-day re-scan is allowed — a returning client whose OR is still being prepared may rescan on any later day until the OR is collected.
